...
type Person = {
  ...
  */
  ...
  console.groupCollapsed('3. Sukurtite masyvą su vardais, pavardėmis ir lytimi, pagal pradinį žmonių masyvą');
  {
    ...
    const Task3Func = ({ name, surname, sex }: Person): Task3 => ({
        name, surname, sex
      });
    
      const result: Task3[] = people.map(Task3Func);
  ...
  const createIdentity = ({ name, surname }: Person): Human => ({ name, surname });

  const identityReducer = (result: Human[], { hasCar, name, surname }: Person): Human[] => {
  ...
  const married = (result: IsMarried[], person: Person): IsMarried[] => {
    if (person.married) result.push(person as IsMarried);

    return result;
  }

  const marriedPeople: IsMarried[] = people.reduce(married, []);
  ...
  */
  ...
  const anonymousPesron: AnonymousPerson[] = people.map(createAnonymous);
  }
  console.groupEnd();
